...
public class RoleAuthorizationService {
    ...
    //튜터 검증
    public void checkTutor(Long channelId, Long userId) {

        ChannelUser channelUser =
                channelUserRepository.findByChannel_ChannelIdAndUserId(channelId, userId)
                        .orElseThrow(() -> new EntityNotFoundException("채널에 속하지 않은 사용자입니다."));

        if (channelUser.getRole().getRoleName() != RoleName.TUTOR) {
