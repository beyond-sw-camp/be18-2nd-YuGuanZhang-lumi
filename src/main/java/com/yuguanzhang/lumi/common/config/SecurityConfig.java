    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.csrf(csrf -> csrf.disable()) // Postman 테스트 시 CSRF 비활성화
                .authorizeHttpRequests(
                        (authorize) -> authorize.requestMatchers("/login", "/sign_up",
                                "/api/sign_up").permitAll().anyRequest().authenticated()
                        // authenticated은 인증된 사용자에 대한 요청만 허가한다.
                )
                //.httpBasic(Customizer.withDefaults())
                .formLogin(form -> form.loginPage("/login").defaultSuccessUrl("/main", true)
